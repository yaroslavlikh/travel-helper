...
Endpoint может не передать время исходного поиска и срок действия записи. В этом случае система
сохраняет время получения ответа, помечает возраст как неизвестный и понижает confidence; запись
не выдаётся за live-цену.
