...
в реальном браузере; панели ограничены viewport, служебная типографика остаётся читаемой, а
мобильные вкладки передают выбранное состояние assistive technologies. `POST /feedback` принят
для локальной продуктовой проверки, но
