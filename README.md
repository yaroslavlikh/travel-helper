...
В отдельном subchat Стамбула вопросы о местах дополнительно получают до пяти релевантных записей
из этого каталога: название, тип, теги и ссылку на первоисточник. Записи заземляют ответ модели,
но не используются для утверждений о текущих часах работы, ценах или доступности. Без каталога
subchat остаётся доступен и честно сообщает, что ответ ограничен данными карточки.
Лицензируемые описания мест импортируются только через reviewed manifest; формат, права на reuse и
команда описаны в [`docs/poi-description-import.md`](docs/poi-description-import.md).
